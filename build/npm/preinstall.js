...
function installHeaders() {
	...
	const quoted_node_gyp = `"${node_gyp}"`;

	const result = cp.execFileSync(quoted_node_gyp, ['list'], { encoding: 'utf8', shell: true });
	...
	if (local !== undefined && !versions.has(local.target)) {
		cp.execFileSync(quoted_node_gyp, ['install', '--dist-url', local.disturl, local.target], { shell: true });
	}

	if (remote !== undefined && !versions.has(remote.target)) {
		cp.execFileSync(quoted_node_gyp, ['install', '--dist-url', remote.disturl, remote.target], { shell: true });
	}
}
