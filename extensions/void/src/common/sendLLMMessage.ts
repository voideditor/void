...
// Gorq
const sendGorqMsg: SendLLMMessageFnTypeInternal = ({
	messages,
	onText,
	onFinalMessage,
	apiConfig,
	...
	const stream = groq.chat.completions
		.create({
		...
		.then(async (responseArr) => {
			...
			for (let response of responseArr) {
				...
				if (type === "message") {
					...
					const { filepath, linestart, lineend } = message as {
						filepath: string;
...
export const sendLLMMessage: SendLLMMessageFnTypeExternal = ({
	...
	} else if (whichApi === "gorq") {
		return sendGorqMsg({ messages, onText, onFinalMessage, apiConfig });	
