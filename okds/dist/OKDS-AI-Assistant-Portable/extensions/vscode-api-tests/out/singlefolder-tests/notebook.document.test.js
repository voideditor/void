...
suite('Notebook Document', function () {
    const simpleContentProvider = new class {
        deserializeNotebook(_data) {
            return new vscode.NotebookData([new vscode.NotebookCellData(vscode.NotebookCellKind.Code, '// SIMPLE', 'javascript')]);
        }
        serializeNotebook(_data) {
    ...
    test('open untitled with data', async function () {
        const nb = await vscode.workspace.openNotebookDocument('notebook.nbdtest', new vscode.NotebookData([
    ...
    test('workspace edit API (replaceCells, event)', async function () {
        ...
        const event = utils.asPromise(vscode.workspace.onDidChangeNotebookDocument);
        const success = await vscode.workspace.applyEdit(edit);
        assert.strictEqual(success, true);
    ...
    test.skip('onDidOpenNotebookDocument - emit event only once when opened in two editors', async function () {
        const uri = await utils.createRandomFile(undefined, undefined, '.nbdtest');
        let counter = 0;
...
//# sourceMappingURL=notebook.document.test.js.map