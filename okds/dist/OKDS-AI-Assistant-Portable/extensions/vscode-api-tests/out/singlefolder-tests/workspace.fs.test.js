...
suite('vscode API - workspace-fs', () => {
    ...
    test('fs.write/stat/read/delete', async function () {
        const uri = root.with({ path: path_1.posix.join(root.path, 'new.file') });
    ...
    test('fs.delete folder', async function () {
        const folder = root.with({ path: path_1.posix.join(root.path, 'folder') });
        const file = root.with({ path: path_1.posix.join(root.path, 'folder/file') });
    ...
    test('vscode.workspace.fs.remove() (and copy()) succeed unexpectedly. #84177 (1)', async function () {
        const entries = await vscode.workspace.fs.readDirectory(root);
        assert.ok(entries.length > 0);
        const someFolder = root.with({ path: path_1.posix.join(root.path, '6b1f9d664a92') });
        try {
    ...
    test('vscode.workspace.fs.remove() (and copy()) succeed unexpectedly. #84177 (2)', async function () {
        ...
        await vscode.workspace.fs.createDirectory(folder);
        await vscode.workspace.fs.writeFile(file, Buffer.from('FOO'));
        const someFolder = root.with({ path: path_1.posix.join(root.path, '6b1f9d664a92/a564c52da70a') });
        try {
    ...
    test('fs.createFolder creates recursively', async function () {
        const folder = root.with({ path: path_1.posix.join(root.path, 'deeply', 'nested', 'folder') });
        await vscode.workspace.fs.createDirectory(folder);
        ...
        const file = root.with({ path: path_1.posix.join(folder.path, 'file.txt') });
        await vscode.workspace.fs.writeFile(file, Buffer.from('Hello World'));
        const folder2 = root.with({ path: path_1.posix.join(file.path, 'invalid') });
        let e;
        try {
        ...
        assert.ok(e);
        const folder3 = root.with({ path: path_1.posix.join(root.path, 'DEEPLY', 'NESTED', 'FOLDER') });
    ...
    test('fs.writeFile creates parents recursively', async function () {
        const folder = root.with({ path: path_1.posix.join(root.path, 'other-deeply', 'nested', 'folder') });
        const file = root.with({ path: path_1.posix.join(folder.path, 'file.txt') });
        await vscode.workspace.fs.writeFile(file, Buffer.from('Hello World'));
...
//# sourceMappingURL=workspace.fs.test.js.map