...
import { ChatMode, FeatureName, ModelSelection, ProviderName } from '../common/orkideSettingsTypes.js';
...
class ConvertToLLMMessageService extends Disposable implements IConvertToLLMMessageService {
	...
	private _getVoidRulesFileContents(): string {
		try {
			...
			for (const folder of workspaceFolders) {
				...
				const { model } = this.orkideModelService.getModel(uri)
				if (!model) continue
	...
	private _getCombinedAIInstructions(): string {
		const globalAIInstructions = this.orkideSettingsService.state.globalSettings.aiInstructions;
	...
	prepareLLMSimpleMessages: IConvertToLLMMessageService['prepareLLMSimpleMessages'] = ({ simpleMessages, systemMessage, modelSelection, featureName }) => {
		...
		const { overridesOfModel } = this.orkideSettingsService.state
		...
		const modelSelectionOptions = this.orkideSettingsService.state.optionsOfModelSelection[featureName][modelSelection.providerName]?.[modelSelection.modelName]
	...
	prepareLLMChatMessages: IConvertToLLMMessageService['prepareLLMChatMessages'] = async ({ chatMessages, chatMode, modelSelection }) => {
		...
		const { disableSystemMessage } = this.orkideSettingsService.state.globalSettings;
		const fullSystemMessage = await this._generateChatMessagesSystemMessage(chatMode, specialToolFormat)
		const systemMessage = disableSystemMessage ? '' : fullSystemMessage;

		const modelSelectionOptions = this.orkideSettingsService.state.optionsOfModelSelection['Chat'][modelSelection.providerName]?.[modelSelection.modelName]
