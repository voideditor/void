...
import { orkidePrefixAndSuffix, ctrlKStream_userMessage, ctrlKStream_systemMessage, defaultQuickEditFimTags, rewriteCode_systemMessage, rewriteCode_userMessage, searchReplaceGivenDescription_systemMessage, searchReplaceGivenDescription_userMessage, tripleTick, } from '../common/prompt/prompts.js';
import { IOrkideCommandBarService } from './orkideCommandBarService.js';
...
import { IOrkideSettingsService } from '../common/orkideSettingsService.js';
import { FeatureName } from '../common/orkideSettingsTypes.js';
...
// import { VOID_OPEN_SETTINGS_ACTION_ID } from './orkideSettingsPane.js';
...
class EditCodeService extends Disposable implements IEditCodeService {
	...
	constructor(
		...
		@IOrkideSettingsService private readonly _settingsService: IOrkideSettingsService,
		// @IFileService private readonly _fileService: IFileService,
		@IOrkideModelService private readonly _orkideModelService: IOrkideModelService,
		...
		const initializeModel = async (model: ITextModel) => {
			...
			await this._orkideModelService.initializeModel(model.uri)
	...
	private _addDiffAreaStylesToURI = (uri: URI) => {
		const { model } = this._orkideModelService.getModel(uri)
	...
	private _addToHistory(uri: URI, opts?: { onWillUndo?: () => void }) {
		...
		const onFinishEdit = async () => {
			...
			await this._orkideModelService.saveModel(uri)
		}
		return { onFinishEdit }
	}
	...
	public async callBeforeApplyOrEdit(givenURI: URI | 'current') {
		...
		if (!uri) return
		await this._orkideModelService.initializeModel(uri)
		await this._orkideModelService.saveModel(uri) // save the URI
	}
	...
	private _initializeWriteoverStream(opts: StartApplyingOpts): [DiffZone, Promise<void>] | undefined {
		...
		if (from === 'ClickApply') {
			...
			const { prefix, suffix } = orkidePrefixAndSuffix({ fullFileStr: originalFileCode, startLine, endLine })
...
class AcceptRejectInlineWidget extends Widget implements IOverlayWidget {
	...
	constructor(
		...
		@IOrkideCommandBarService private readonly _orkideCommandBarService: IOrkideCommandBarService,
		...
		const getAcceptRejectText = () => {
			...
			const commandBarStateAtUri = this._orkideCommandBarService.stateOfURI[uri.fsPath];
		...
		this._register(this._orkideCommandBarService.onDidChangeState(e => {
