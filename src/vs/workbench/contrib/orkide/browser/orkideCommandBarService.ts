...
export interface IOrkideCommandBarService {
...
export const IOrkideCommandBarService = createDecorator<IOrkideCommandBarService>('VoidCommandBarService');
...
export class OrkideCommandBarService extends Disposable implements IOrkideCommandBarService {
	...
	async goToURIIdx(idx: number | null): Promise<void> {
		...
		const { model } = await this._orkideModelService.getModelSafe(nextURI);
		if (!model) return;
...
registerSingleton(IOrkideCommandBarService, VoidCommandBarService, InstantiationType.Delayed); // delayed is needed here :(
...
registerAction2(class extends Action2 {
	...
	async run(accessor: ServicesAccessor): Promise<void> {
		...
		const commandBarService = accessor.get(IOrkideCommandBarService);
