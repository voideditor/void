...
export const IconShell1 = ({ onClick, Icon, disabled, className, ...props }: IconButtonProps & React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  ...
  return <button
    ...
    className={` void-size-[18px] void-p-[2px] void-flex void-items-center void-justify-center void-text-sm void-text-void-fg-3 hover:void-brightness-110 disabled:void-opacity-50 disabled:void-cursor-not-allowed ${
...
export const StatusIndicator = ({ indicatorColor, title, className, ...props }: {indicatorColor: IndicatorColor;title?: React.ReactNode;className?: string;} & React.HTMLAttributes<HTMLDivElement>) => {
  return (
    <div className={`void-flex void-flex-row void-text-void-fg-3 void-text-xs void-items-center void-gap-1.5 ${className}`} {...props}>
			{title && <span className="void-opacity-80">{title}</span>}
			<div
        className={` void-size-1.5 void-rounded-full void-border ${
        indicatorColor === 'dark' ? "void-bg-[rgba(0,0,0,0)] void-border-void-border-1" :
        indicatorColor === 'orange' ? "void-bg-orange-500 void-border-orange-500 void-shadow-[0_0_4px_0px_rgba(234,88,12,0.6)]" :
        indicatorColor === 'green' ? "void-bg-green-500 void-border-green-500 void-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" :
        indicatorColor === 'yellow' ? "void-bg-yellow-500 void-border-yellow-500 void-shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]" : "void-bg-void-border-1 void-border-void-border-1"} `} />
...
}: {codeStr: string;applyBoxId: string;language?: string;uri: URI | 'current';}) => {
  const accessor = useAccessor();
  const metricsService = accessor.get('IMetricsService');
...
}: {codeStr: string;applyBoxId: string;} & ({uri: URI;type: 'edit_file' | 'rewrite_file';threadId: string;})) =>
{
  const accessor = useAccessor();
  const editCodeService = accessor.get('IEditCodeService');
  const metricsService = accessor.get('IMetricsService');
...
export const BlockCodeApplyWrapper = ({
...
}: {codeStr: string;children: React.ReactNode;applyBoxId: string;canApply: boolean;language: string;uri: URI | 'current';}) => {
  const accessor = useAccessor();
  const commandService = accessor.get('ICommandService');
  const { currStreamStateRef } = useApplyStreamState({ applyBoxId });
  const currStreamState = currStreamStateRef.current;
  ...
  <ListableToolItem
    name={<span className="void-not-italic">{getBasename(uri.fsPath)}</span>}
    isSmall={true}
    showDot={false}
  ...
  return <div className="void-border void-border-void-border-3 void-rounded void-overflow-hidden void-bg-void-bg-3 void-my-1">
		{/* header */}
		<div className=" void-select-none void-flex void-justify-between void-items-center void-py-1 void-px-2 void-border-b void-border-void-border-3 void-cursor-default">
			<div className="void-flex void-items-center">
				<StatusIndicatorForApplyButton uri={uri} applyBoxId={applyBoxId} />
				<span className="void-text-[13px] void-font-light void-text-void-fg-3">
					{name}
				</span>
			</div>
			<div className={`${canApply ? "" : "void-hidden"} void-flex void-items-center void-gap-1`}>
