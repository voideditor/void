...
const LatexRender = ({ latex }: {latex: string;}) => {
  return <span className="void-katex-error void-text-red-500">{latex}</span>;
  // try {
  // 	let formula = latex;
...
const CodespanWithLink = ({ text, rawText, chatMessageLocation }: {text: string;rawText: string;chatMessageLocation: ChatMessageLocation;}) => {
  ...
  const onClick = () => {
    ...
    if (link.selection)
    voidOpenFileFn(link.uri, accessor, [link.selection.startLineNumber, link.selection.endLineNumber]);else
  ...
  return <Codespan
    text={displayText}
    onClick={onClick}
    className={link ? "void-underline hover:void-brightness-90 void-transition-all void-duration-200 void-cursor-pointer" : ""}
...
const RenderToken = ({ token, inPTag, codeURI, chatMessageLocation, tokenIdx, ...options }: {token: Token | string;inPTag?: boolean;codeURI?: URI;chatMessageLocation?: ChatMessageLocation;tokenIdx: string;} & RenderTokenOptions): React.ReactNode => {
  ...
  if (t.type === 'code') {
    ...
    {// no language provided - fallback - get lang from the uri and contents
  ...
  if (t.type === 'paragraph') {
    ...
    if (latexSegments.length !== 0) {
      if (inPTag) {
        return <span className="void-block">{latexSegments}</span>;
      }
    ...
    if (inPTag) return <span className="void-block">{contents}</span>;
    return <p>{contents}</p>;
  }
  ...
  return (
    <div className="void-bg-orange-50 void-rounded-sm void-overflow-hidden void-p-2">
			<span className="void-text-sm void-text-orange-500">Unknown token rendered...</span>
		</div>);
