...
const getOptionsAtPath = async (accessor: ReturnType<typeof useAccessor>, path: string[], optionText: string): Promise<Option[]> => {
  ...
  if (path.length === 0 && optionText.trim().length > 0) {// (special case): directly search for both files and folders if optionsPath is empty and there's a search term
...
export const VoidInputBox2 = forwardRef<HTMLTextAreaElement, InputBox2Props>(function X({ initValue, placeholder, multiline, enableAtToMention, fnsRef, className, onKeyDown, onFocus, onBlur, onChangeText }, ref) {
  ...
  const onSelectOption = async () => {
    ...
    if (isLastOption) {
      ...
      if (option.leafNodeType === 'Folder') newSelection = {
        type: 'Folder',
        uri: option.uri,
        language: undefined,
        state: undefined
  ...
  const onMenuKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === 'ArrowUp') {
      ...
      if (isCommandKeyPressed) {// Ctrl+Backspace
        onPathTextChange('');
      } else
      {// Backspace
      ...
      if (isCommandKeyPressed) {
        ...
        if (isTypingEnabled) {onPathTextChange(optionText + e.key);
        }
      }
    }
  ...
  return <>
    ...
    <div
      ...
      onWheel={(e) => e.stopPropagation()}>
				...
				{/* Breadcrumbs Header */}
				{isBreadcrumbsShowing && <div className="void-px-2 void-py-1 void-text-void-fg-1 void-bg-void-bg-2-alt void-border-b void-border-void-border-3 void-sticky void-top-0 void-bg-void-bg-1 void-z-10 void-select-none void-pointer-events-none">
					{optionText ?
        ...
        <div className="void-opacity-50">Enter text to filter...</div>
        }
				</div>}


				{/* Options list */}
				<div className="void-max-h-[400px] void-w-full void-max-w-full void-overflow-y-auto void-overflow-x-auto">
					<div className="void-w-max void-min-w-full void-flex void-flex-col void-gap-0 void-text-nowrap void-flex-nowrap">
						{options.length === 0 ?
          <div className="void-text-void-fg-3 void-px-3 void-py-0.5">No results found</div> :
          options.map((o, oIdx) => {
            ...
            return (
              ...
              <div
                ...
                key={o.fullName}
                className={` void-flex void-items-center void-gap-2 void-px-3 void-py-1 void-cursor-pointer ${


                oIdx === optionIdx ? "void-bg-blue-500 void-text-white/80" : "void-bg-void-bg-2-alt void-text-void-fg-1"} `}
                ...
                onMouseMove={() => {setOptionIdx(oIdx);}}>
										...
										{o.fullName && o.fullName !== o.abbreviatedName && <span className="void-opacity-60 void-text-sm">{o.fullName}</span>}
...
export const VoidSimpleInputBox = ({ value, onChangeValue, placeholder, className, disabled, passwordBlur, compact, ...inputProps
...
}: {value: string;onChangeValue: (value: string) => void;placeholder: string;className?: string;disabled?: boolean;compact?: boolean;passwordBlur?: boolean;} & React.InputHTMLAttributes<HTMLInputElement>) => {
  ...
  return (
    <input
      ...
      placeholder={placeholder}
      disabled={disabled}
      className={`void-w-full void-resize-none void-bg-void-bg-1 void-text-void-fg-1 placeholder:void-text-void-fg-3 void-border void-border-void-border-2 focus:void-border-void-border-1 ${
      compact ? "void-py-1 void-px-2" : "void-py-2 void-px-4 "} void-rounded ${

      disabled ? "void-opacity-50 void-cursor-not-allowed" : ""} ${
      className}`}
      style={{
...
}: {onChangeText: (value: string) => void;styles?: Partial<IInputBoxStyles>;onCreateInstance?: (instance: InputBox) => void | IDisposable[];inputBoxRef?: {current: InputBox | null;};placeholder: string;isPasswordField?: boolean;multiline: boolean;}) => {

  const accessor = useAccessor();

  const contextViewProvider = accessor.get('IContextViewService');
  return <WidgetComponent
    className=" void-bg-void-bg-1 void-force-child-placeholder-void-fg-1 "
...
export const VoidSlider = ({
...
}: {value: number;onChange: (value: number) => void;disabled?: boolean;size?: 'xxs' | 'xs' | 'sm' | 'sm+' | 'md';min?: number;max?: number;step?: number;className?: string;width?: number;}) => {
  ...
  return (
    <div className={`void-inline-flex void-items-center void-flex-shrink-0 ${className}`}>
			{/* Outer container with padding to account for thumb overhang */}
			<div className={`void-relative void-flex-shrink-0 ${disabled ? "void-opacity-25" : ""}`}
      style={{
        width
        ...
        // paddingRight: thumbSizePx / 2
            ...
            onClick={handleTrackClick} />
					...
					{/* Track */}
					<div
            className={`void-relative ${size === 'xxs' ? "void-h-0.5" :
            size === 'xs' ? "void-h-1" :
            size === 'sm' ? "void-h-1.5" :
            size === 'sm+' ? "void-h-2" : "void-h-2.5"} void-bg-void-bg-2 void-rounded-full void-cursor-pointer`}

            onClick={handleTrackClick}>

						{/* Filled part of track */}
						<div
              className={`void-absolute void-left-0 ${size === 'xxs' ? "void-h-0.5" :
              size === 'xs' ? "void-h-1" :
              size === 'sm' ? "void-h-1.5" :
              size === 'sm+' ? "void-h-2" : "void-h-2.5"} void-bg-void-fg-1 void-rounded-full`}
              ...
              style={{ width: `${percentage}%` }} />
					...
					{/* Thumb */}
					<div
            className={`void-absolute void-top-1/2 void-transform -void-translate-x-1/2 -void-translate-y-1/2 ${
            size === 'xxs' ? "void-h-2 void-w-2" :
            size === 'xs' ? "void-h-2.5 void-w-2.5" :
            size === 'sm' ? "void-h-3 void-w-3" :
            size === 'sm+' ? "void-h-3.5 void-w-3.5" : "void-h-4 void-w-4"} void-bg-void-fg-1 void-rounded-full void-shadow-md ${

            disabled ? "void-cursor-not-allowed" : "void-cursor-grab active:void-cursor-grabbing"} void-border void-border-void-fg-1`}
...
}: {value: boolean;onChange: (value: boolean) => void;disabled?: boolean;size?: 'xxs' | 'xs' | 'sm' | 'sm+' | 'md';}) => {
  return (
    <label className="void-inline-flex void-items-center" {...props}>
			<div
        onClick={() => !disabled && onChange(!value)}
        className={` void-cursor-pointer void-relative void-inline-flex void-items-center void-rounded-full void-transition-colors void-duration-200 void-ease-in-out ${


        value ? "void-bg-zinc-900 dark:void-bg-white" : "void-bg-white dark:void-bg-zinc-600"} ${
        disabled ? "void-opacity-25" : ""} ${
        size === 'xxs' ? "void-h-3 void-w-5" : ""} ${
        ...
        size === 'md' ? "void-h-6 void-w-11" : ""} `}>
				...
				<span
          className={` void-inline-block void-transform void-rounded-full void-bg-white dark:void-bg-zinc-900 void-shadow void-transition-transform void-duration-200 void-ease-in-out ${
          ...
          size === 'xxs' ? value ? "void-translate-x-2.5" : "void-translate-x-0.5" : ""} ${
...
}: {options: T[];selectedOption: T | undefined;onChangeOption: (newValue: T) => void;getOptionDropdownName: (option: T) => string;getOptionDropdownDetail?: (option: T) => string;getOptionDisplayName: (option: T) => string;getOptionsEqual: (a: T, b: T) => boolean;className?: string;arrowTouchesText?: boolean;matchInputWidth?: boolean;gapPx?: number;offsetPx?: number;}) => {
  ...
  return (
    <div className={`void-inline-block void-relative ${className}`}>
        ...
        aria-hidden="true">
          ...
          return (
            <div key={optionName + optionDetail} className="void-flex void-items-center void-whitespace-nowrap">
							<div className="void-w-4" />
							<span className="void-flex void-justify-between void-w-full">
        ...
        onClick={() => setIsOpen(!isOpen)}>
				...
				<span className={`void-truncate ${arrowTouchesText ? "void-mr-1" : ""}`}>
      ...
      <div
        ...
        onWheel={(e) => e.stopPropagation()}>
        <div className="void-overflow-auto void-max-h-80">
            ...
            return (
              <div
                key={optionName}
                className={`void-flex void-items-center void-px-2 void-py-1 void-pr-4 void-cursor-pointer void-whitespace-nowrap void-transition-all void-duration-100 ${

                thisOptionIsSelected ? "void-bg-blue-500 void-text-white/80" : "hover:void-bg-blue-500 hover:void-text-white/80"} `}
                ...
                onClick={() => {
                  ...
                  <svg className="void-size-3" viewBox="0 0 12 12" fill="none">
                      ...
                      strokeLinejoin="round" />
									...
									<span className="void-flex void-justify-between void-items-center void-w-full void-gap-x-1">
										<span>{optionName}</span>
										<span className="void-opacity-60">{optionDetail}</span>
									</span>
								</div>);
...
const SingleDiffEditor = ({ block, lang }: {block: ExtractedSearchReplaceBlock;lang: string | undefined;}) => {
  ...
  return (
    <div className="void-w-full void-bg-void-bg-3 bg-editor-style-override" ref={divRef} />);
...
export const VoidDiffEditor = ({ uri, searchReplaceBlocks, language }: {uri?: any;searchReplaceBlocks: string;language?: string;}) => {
  ...
  if (blocks.length === 0) {
    return <div className="void-w-full void-p-4 void-text-void-fg-4 void-text-sm">No changes found</div>;
  }

  // Display all blocks
  return (
    <div className="void-w-full void-flex void-flex-col void-gap-2">
			{blocks.map((block, index) =>
      <div key={index} className="void-w-full">
					{blocks.length > 1 &&
        <div className="void-text-void-fg-4 void-text-xs void-mb-1 void-px-1">
