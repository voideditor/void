...
import { MCPUserStateOfName } from './orkideSettingsTypes.js';
...
class MCPService extends Disposable implements IMCPService {
	...
	private async _initialize() {
		try {
			await this.orkideSettingsService.waitForInitState;
	...
	private async _refreshMCPServers(): Promise<void> {
		...
		await this.orkideSettingsService.addMCPUserStateOfNames(addedUserStateOfName);

		// delete isOn for any servers that no longer show up in the config
		await this.orkideSettingsService.removeMCPUserStateOfNames(removedServerNames);
		...
		this.channel.call('refreshMCPServers', {
			...
			removedServerNames,
			updatedServerNames,
			userStateOfName: this.orkideSettingsService.state.mcpUserStateOfName,
		})
	}
	...
	public async toggleServerIsOn(serverName: string, isOn: boolean): Promise<void> {
		...
		await this.orkideSettingsService.setMCPServerState(serverName, { isOn });
