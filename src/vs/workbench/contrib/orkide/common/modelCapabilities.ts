...
import { FeatureName, ModelSelectionOptions, OverridesOfModel, ProviderName } from './orkideSettingsTypes.js';
...
type VoidStaticProviderInfo = { // doesn't change (not stateful)
	...
	modelOptions: { [key: string]: OrkideStaticModelInfo };
	modelOptionsFallback: (modelName: string, fallbackKnownValues?: Partial<OrkideStaticModelInfo>) => (OrkideStaticModelInfo & { modelName: string, recognizedModelName: string }) | null;
}
...
const defaultModelOptions = {
	...
	supportsFIM: false,
	reasoningCapabilities: false,
} as const satisfies OrkideStaticModelInfo
...
} as const satisfies { [s: string]: Partial<OrkideStaticModelInfo> }
...
const extensiveModelOptionsFallback: VoidStaticProviderInfo['modelOptionsFallback'] = (modelName, fallbackKnownValues) => {
	...
	const toFallback = <T extends { [s: string]: Omit<OrkideStaticModelInfo, 'cost' | 'downloadable'> },>(obj: T, recognizedModelName: string & keyof T)
		: OrkideStaticModelInfo & { modelName: string, recognizedModelName: string } => {
...
} as const satisfies { [s: string]: OrkideStaticModelInfo }
...
} as const satisfies Record<string, OrkideStaticModelInfo>
