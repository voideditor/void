...
export class OrkideMainUpdateService extends Disposable implements IOrkideUpdateService {
	...
	async check(explicit: boolean): Promise<OrkideCheckUpdateRespose> {
	...
	private async _manualCheckGHTagIfDisabled(explicit: boolean): Promise<OrkideCheckUpdateRespose> {
		try {
