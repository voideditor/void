...
import {
	...
	voidOpenFileFn,
	ToolChildrenWrapper,
} from "../sidebar-tsx/SidebarChat.js";
...
export const CopyButton = ({
	...
	codeStr: string | (() => Promise<string> | string);
	toolTipName: string;
}) => {
	...
	const onCopy = useCallback(async () => {
		clipboardService
			.writeText(typeof codeStr === "string" ? codeStr : await codeStr())
			.then(() => {
				setCopyButtonText(CopyButtonText.Copied);
			})
			.catch(() => {
				setCopyButtonText(CopyButtonText.Error);
			});
		...
		<IconShell1
			Icon={
				copyButtonText === CopyButtonText.Copied
					? Check
					: copyButtonText === CopyButtonText.Error
						? X
						: Copy
			}
			onClick={onCopy}
...
export const JumpToTerminalButton = ({ onClick }: { onClick: () => void }) => {
	return <IconShell1 Icon={Terminal} onClick={onClick} />;
};

// state persisted for duration of react only
// TODO change this to use type `ChatThreads.applyBoxState[applyBoxId]`
const _applyingURIOfApplyBoxIdRef: {
	current: { [applyBoxId: string]: URI | undefined };
} = { current: {} };
...
export const StatusIndicator = ({
	...
	return (
		<div
			className={`flex flex-row text-void-fg-3 text-xs items-center gap-1.5 ${className}`}
			{...props}
		>
			...
			<div
				className={` size-1.5 rounded-full border
					${
						indicatorColor === "dark"
							? "bg-[rgba(0,0,0,0)] border-void-border-1"
							: indicatorColor === "orange"
								? "bg-orange-500 border-orange-500 shadow-[0_0_4px_0px_rgba(234,88,12,0.6)]"
								: indicatorColor === "green"
									? "bg-green-500 border-green-500 shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]"
									: indicatorColor === "yellow"
										? "bg-yellow-500 border-yellow-500 shadow-[0_0_4px_0px_rgba(22,163,74,0.6)]"
...
export const useEditToolStreamState = ({
	applyBoxId,
	uri,
}: {
	applyBoxId: string;
	...
	const [streamState, setStreamState] = useState(
		voidCommandBarService.getStreamState(uri),
	);
	// listen for stream updates on this box
	useCommandBarURIListener(
		useCallback(
...
export const StatusIndicatorForApplyButton = ({
	applyBoxId,
	uri,
}: {
	applyBoxId: string;
	...
	const color =
		currStreamState === "idle-no-changes"
			...
			: currStreamState === "streaming"
				? "orange"
				: currStreamState === "idle-has-changes"
					? "green"
					: null;
	...
	const tooltipName =
		currStreamState === "idle-no-changes"
			...
			: currStreamState === "streaming"
				? "Applying"
				: currStreamState === "idle-has-changes"
					? "Done" // also 'Done'? 'Applied' looked bad
					: "";
...
const ApplyButtonsForTerminal = ({
	...
	const onClickSubmit = useCallback(async () => {
		...
		try {
			...
			const terminalId = await terminalToolService.createPersistentTerminal({
				cwd: null,
			});
			const { interrupt } = await terminalToolService.runCommand(codeStr, {
				type: "persistent",
...
const ApplyButtonsForEdit = ({
	...
	const settingsState = useSettingsState();
	const isDisabled =
		!!isFeatureNameDisabled("Apply", settingsState) || !applyBoxId;

	const { currStreamStateRef, setApplying } = useApplyStreamState({
		applyBoxId,
	});
	...
	const onClickSubmit = useCallback(async () => {
		...
		const [newApplyingUri, applyDonePromise] =
		...
		if (!applyDonePromise) {
			notificationService.info(
				`Void Error: We couldn't run Apply here. ${uri === "current" ? "This Apply block wants to run on the current file, but you might not have a file open." : `This Apply block wants to run on ${uri.fsPath}, but it might not exist.`}`,
			);
		}
		...
		applyDonePromise?.catch((e) => {
			...
			notificationService.info(
				`Void Error: There was a problem running Apply: ${e}.`,
			);
		});
...
export const EditToolAcceptRejectButtonsHTML = ({
	...
	const isDisabled =
		!!isFeatureNameDisabled("Chat", settingsState) || !applyBoxId;
...
export const BlockCodeApplyWrapper = ({
	...
	return (
		<div className="border border-void-border-3 rounded-sm overflow-hidden bg-void-bg-3 my-1">
			{/* header */}
			<div className=" select-none flex justify-between items-center py-1 px-2 border-b border-void-border-3 cursor-default">
				<div className="flex items-center">
					...
					<span className="text-[13px] font-light text-void-fg-3">{name}</span>
				</div>
			...
			{/* contents */}
			<ToolChildrenWrapper>{children}</ToolChildrenWrapper>
		</div>
	);
};
