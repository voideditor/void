...
} from "../../../../../../../platform/void/common/llmMessageTypes.js";
...
export const sendLLMMessage = ({
	...
	const captureChatEvent = (eventId: string, extras?: object) => {
		posthog.capture(eventId, {
			...
			messagesShape: messages?.map((msg) => ({
				role: msg.role,
	...
	const onFinalMessage: OnFinalMessage = ({ fullText }) => {
		if (_didAbort) return;
		captureChatEvent(`${loggingName} - Received Full Message`, {
	...
	const onAbort = () => {
		captureChatEvent(`${loggingName} - Abort`, {
			messageLengthSoFar: _fullTextSoFar.length,
		});
		_aborter?.();
		_didAbort = true;
	};
	...
	captureChatEvent(`${loggingName} - Sending Message`, {
	...
	try {
		switch (voidConfig.default.whichApi) {
			...
			default:
				onError({
					error: `Error: whichApi was "${voidConfig.default.whichApi}", which is not recognized!`,
				});
				break;
		}
	} catch (error) {
		if (error instanceof Error) {
			...
			onError({ error: `Unexpected Error in sendLLMMessage: ${error}` });
		}
		(_aborter as any)?.();
		_didAbort = true;
	}
