		...
		<div className="flex flex-col md:flex-row w-full h-[80vh] gap-6 max-w-[900px] mx-auto relative">
			...
			<div className="md:w-1/4 w-full flex flex-col gap-6 p-6 border-none border-void-border-2 h-full overflow-y-auto">
				...
				<div className="flex md:flex-col gap-2">
					{[...tabNames, "Cloud/Other"].map((tab) => (
						<button
							key={tab}
							className={`py-2 px-4 text-left border-l-2 transition-all duration-200 ${
								currentTab === tab
									? "border-void-fg-1 text-void-fg-1 font-medium bg-void-bg-1"
									: "border-transparent text-void-fg-3 hover:text-void-fg-1 hover:bg-void-bg-2"
							}`}
							onClick={() => {
			...
			<div className="flex-1 flex flex-col items-center justify-start p-6 h-full overflow-y-auto">
				...
				{(currentTab === "Local" || currentTab === "Cloud/Other") && (
					<div className="w-full max-w-xl mt-8 bg-void-bg-2/50 rounded-sm p-6 border border-void-border-4">
...
const NextButton = ({
	...
	return (
		<button
			...
			className={`px-6 py-2 border border-void-border-2 ${
				disabled
					? "bg-void-bg-1 opacity-50 cursor-not-allowed"
					: "hover:bg-void-bg-2 bg-void-bg-1"
			} rounded-sm text-void-fg-1 duration-200 transition-all
			`}
			{...(disabled && {
...
const PreviousButton = ({
	...
	return (
		<button
			onClick={onClick}
			className="px-6 py-2 rounded-sm border border-transparent hover:border-void-border-2 text-void-fg-2 hover:text-void-fg-1 duration-200 transition-all"
			{...props}
		>
			Back
		</button>
...
const OnboardingPageShell = ({
	...
	return (
		<div
			className={`h-[80vh] text-lg flex flex-col gap-4 w-full mx-auto ${hasMaxWidth ? "max-w-[600px]" : ""} ${className}`}
		>
	...
	return (
		<button
			...
			className={`
				...
				rounded-sm
				border border-void-border-2 hover:border-void-fg-1
				group
				${className}
			`}
			{...props}
